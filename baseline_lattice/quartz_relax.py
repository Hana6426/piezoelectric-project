...
from baseline_lattice.structures import make_quartz_struc

def relax_once(nk=6, ecut=60):
    pseudos_dir = "./pseudos"
    pseudopots = {
        "Si": PseudoPotential(ptype="nc",   element="Si", functional="LDA", name="Si.pz-vbc.UPF"),
        "O" : PseudoPotential(ptype="uspp", element="O",  functional="LDA", name="O.pz-van_ak.UPF"),
    }
    struc = make_quartz_struc()
    kpts = Kpoints(gridsize=[nk, nk, nk], option="automatic", offset=False)

    runpath = Dir(path=os.path.join("./tmp_baselines/SiO2_relaxations", f"SiO2_nk{nk}_ecut{ecut}"))
